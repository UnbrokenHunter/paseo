...
const EXECUTABLE_NAME = "PaseoPlus";
...
async function waitForPackagedAppPage(browser, deadline) {
  while (Date.now() < deadline) {
    const page = browser
      ...
      .find((candidate) => candidate.url().startsWith("paseoplus://app/"));
    if (page) {
      return page;
    }
    await delay(250);
  }
  throw new Error("Timed out waiting for the packaged paseoplus://app/ renderer");
}
