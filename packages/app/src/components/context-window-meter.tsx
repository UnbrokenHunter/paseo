...
import {
  ProviderUsageMeterLabel,
  ProviderUsageMeterTooltipContent,
  useProviderUsageMeterContent,
} from "@/provider-usage/meter-content";
...
export function ContextWindowMeter({
  ...
  const providerUsage = useProviderUsageMeterContent({ provider, serverId });
  ...
  const { barLabel, isBarSkeleton } = providerUsage;
  ...
  const { brandColors } = providerUsage;
  const accessibilityLabel = meterAccessibilityLabel(roundedPercentage, barLabel, t);

  return (
    <Tooltip
      open={providerUsage.isTooltipOpen}
      onOpenChange={providerUsage.handleOpenChange}
      delayDuration={0}
      enabledOnDesktop
      enabledOnMobile
      pinnable
    >
      <TooltipTrigger asChild triggerRefProp="ref">
        <Pressable
          ...
          onPress={providerUsage.handlePress}
        >
          {showBar ? (
            <>
              <View style={[styles.brandDot, { backgroundColor: brandColors.accent }]} />
              <ProviderUsageMeterLabel
                fadeStyle={providerUsage.fadeStyle}
                label={barLabel}
