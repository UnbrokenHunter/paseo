...
function pickAppSettingUpdates(updates: Partial<Settings>): Partial<AppSettings> {
  ...
  if (updates.providerUsageRotation !== undefined) {
    appUpdates.providerUsageRotation = updates.providerUsageRotation;
  }
  return appUpdates;
}
...
export function useSettings<TSelected>(
  ...
  const updateSettings = useCallback(
    async (updates: Partial<Settings>) => {
      const appUpdates = pickAppSettingUpdates(updates);
