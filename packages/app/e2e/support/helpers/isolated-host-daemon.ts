...
export async function startIsolatedHostDaemon(
  ...
  // Run the tsx CLI entry under this Node rather than shelling out to `which tsx`/the
  // .bin shim, which is a .cmd file on Windows and so is not spawnable without a shell.
  ...
  const spawnDaemon = async (): Promise<ChildProcess> => {
    ...
    const child = publishedPackageRoot
      ...
      : spawn(
          process.execPath,
          [tsxCliPath, "scripts/supervisor-entrypoint.ts", "--dev"],
          spawnOptions,
        );
